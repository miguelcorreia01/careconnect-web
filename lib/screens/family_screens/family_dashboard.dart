...
class _FamilyDashboardState extends State<FamilyDashboard> {
  ...
  Future<void> linkPatientToFamily(String email) async {
    try {
      final querySnapshot =
      ...
      await FirebaseFirestore.instance.collection('family_patients').add({
  ...
  Future<void> fetchLinkedPatients() async {
    final currentUser = FirebaseAuth.instance.currentUser;
    final familyId = currentUser!.uid;
    ...
    if (familySnapshot.exists) {
      setState(() {
        familyName = familySnapshot.data()!['name'] ?? '';
      });
    }
    ...
    for (var doc in links.docs) {
      ...
      if (seenPatientIds.contains(patientId)) continue;
...
class FamilyNotificationDropdown extends StatefulWidget {
  @override
  _FamilyNotificationDropdownState createState() =>
      _FamilyNotificationDropdownState();
}

class _FamilyNotificationDropdownState
    extends State<FamilyNotificationDropdown> {
      ...
      builder: (context, snapshot) {
        ...
        return Stack(
          children: [
            ...
            if (docs.where((d) => !(d['read'] ?? false)).isNotEmpty)
              Positioned(
                right: 6,
                top: 6,
                child: Container(
                  ...
                  child: Text(
                    '${docs.where((d) => !(d['read'] ?? false)).length}',
